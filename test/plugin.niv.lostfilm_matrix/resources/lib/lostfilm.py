...
addon = xbmcaddon.Addon(id='plugin.niv.lostfilm_matrix')
...
#========================#========================#========================#
    def create_context(self, serial_id='', se_code='', ismovie=False):
        ...
        if 'search_part' in self.params['mode'] and self.params['param'] == '':
            context_menu.append(('[COLOR=red]Очистить историю[/COLOR]', 'Container.Update("plugin://plugin.niv.lostfilm_matrix/?mode=clean_part")'))

        if serial_id and self.params['mode'] in ('common_part','favorites_part','catalog_part','schedule_part','search_part','serials_part'):
            if not ismovie:
                context_menu.append(('[COLOR=cyan]Избранное - Добавить \ Удалить [/COLOR]', 'Container.Update("plugin://plugin.niv.lostfilm_matrix/?mode=favorites_part&serial_id={}")'.format(serial_id)))

        if serial_id:
            context_menu.append(('[COLOR=white]Обновить описание[/COLOR]', 'Container.Update("plugin://plugin.niv.lostfilm_matrix/?mode=update_serial&id={}&ismovie={}")'.format(serial_id, ismovie)))

        if se_code and not '999' in serial_episode:
            if not ismovie:
                context_menu.append(('[COLOR=blue]Перейти к Сериалу[/COLOR]', 'Container.Update("plugin://plugin.niv.lostfilm_matrix/?mode=select_part&id={}&code={}001999")'.format(serial_id,serial_image)))
            context_menu.append(('[COLOR=white]Открыть торрент файл[/COLOR]', 'Container.Update("plugin://plugin.niv.lostfilm_matrix/?mode=torrent_part&id={}&code={}")'.format(serial_id,se_code)))
            context_menu.append(('[COLOR=yellow]Отметить как просмотренное[/COLOR]', 'Container.Update("plugin://plugin.niv.lostfilm_matrix/?mode=mark_part&param=on&id={}")'.format(se_code)))
            context_menu.append(('[COLOR=yellow]Отметить как непросмотренное[/COLOR]', 'Container.Update("plugin://plugin.niv.lostfilm_matrix/?mode=mark_part&param=off&id={}")'.format(se_code)))

        context_menu.append(('[COLOR=lime]Новости обновлений[/COLOR]', 'Container.Update("plugin://plugin.niv.lostfilm_matrix/?mode=information_part&param=news")'))
        context_menu.append(('[COLOR=darkorange]Обновить Авторизацию[/COLOR]', 'Container.Update("plugin://plugin.niv.lostfilm_matrix/?mode=update_authorization")'))
        context_menu.append(('[COLOR=darkorange]Обновить Базу Данных[/COLOR]', 'Container.Update("plugin://plugin.niv.lostfilm_matrix/?mode=update_database")'))
        context_menu.append(('[COLOR=darkorange]Обновить Прокси[/COLOR]', 'Container.Update("plugin://plugin.niv.lostfilm_matrix/?mode=update_proxy_data")'))
        return context_menu
#========================#========================#========================#
...
#========================#========================#========================#
    ...
    - Версия для Kodi-19\n\
    - Разработка данного плагина прекращена (дальше только для Kodi-20)\n\
    \n\
    Буду поддерживать базовую работоспособность, по возможности'
