...
class Anidub:
    def __init__(self):
        ...
        self.site_url = addon.getSetting('torrent_mirror_0')
#========================#========================#========================#
        from network import WebTools
        self.network = WebTools(
            auth_usage=bool(addon.getSetting('torrent_authmode') == '1'),
            auth_status=bool(addon.getSetting('torrent_auth') == 'true'),
...
#========================#========================#========================#
    def create_context(self, anime_id, title=None):
        ...
        #         context_menu.append(('Искать на Online Сайте', 'Container.Update("plugin://plugin.niv.anidub/?mode=search_part&param=search_string&search_string={}&portal=anidub_o")'.format(title)))
        ...
        #     if 'common_part' in self.params['mode'] or 'favorites_part' in self.params['mode'] or 'search_part' in self.params['mode'] and not self.params['param'] == '':
        #         context_menu.append(('Избранное - Добавить', 'Container.Update("plugin://plugin.niv.anidub/?mode=favorites_part&param=plus&id={}")'.format(anime_id)))
        #         context_menu.append(('Избранное - Удалить', 'Container.Update("plugin://plugin.niv.anidub/?mode=favorites_part&param=minus&id={}")'.format(anime_id)))

        context_menu.append(('Обновить Базу Данных', 'Container.Update("plugin://plugin.niv.anidub/?mode=update_file_part")'))
        context_menu.append(('Обновить Авторизацию', 'Container.Update("plugin://plugin.niv.anidub/?mode=authorization_part&param=update")'))
        #context_menu.append(('Обновить Прокси', 'Container.Update("plugin://plugin.niv.anidub/?mode=proxy_data&param=renew")'))
...
#========================#========================#========================#
    def exec_authorization_part(self):
        if '0' in addon.getSetting('torrent_authmode'):
            return False
        ...
        if 'update' in self.params['param']:
            addon.setSetting('torrent_auth', 'false')
            addon.setSetting('torrent_session', '')
            
        try:
            temp_session = float(addon.getSetting('torrent_session'))
        except:
            temp_session = 0
        
        if time.time() - temp_session > 86400:
            addon.setSetting('torrent_session', str(time.time()))
            try:
        ...
        if not authorization:
            ...
            addon.setSetting('torrent_auth', str(authorization).lower())

        return authorization
#========================#========================#========================#
    def create_proxy_data(self):
        if '0' in addon.getSetting('torrent_unblock'):
            return None

        if '1' in addon.getSetting('torrent_unblock'):
            proxy_data = {'https': addon.getSetting('torrent_unblock_1')}
            return proxy_data

        try:
            proxy_time = float(addon.getSetting('torrent_proxytime'))
        except:
            proxy_time = 0
        ...
        if time.time() - proxy_time > 604800:
            addon.setSetting('torrent_proxytime', str(time.time()))
            ...
            addon.setSetting('torrent_proxy', proxy)
            proxy_data = {'https': proxy}

        else:
            if addon.getSetting('torrent_proxy'):
                proxy_data = {'https': addon.getSetting('torrent_proxy')}
            else:
                content = network.get_web(url=pac_url, bytes=False)
...
#========================#========================#========================#
    def exec_update_proxy_data(self):
        addon.setSetting('torrent_proxy', '')
        addon.setSetting('torrent_proxytime', '')
...
#========================#========================#========================#
    def exec_clean_part(self):
        try:
            addon.setSetting('torrent_search', '')
...
#========================#========================#========================#
    def exec_search_part(self):
        if not self.params['param']:
            ...
            data_array = addon.getSetting('torrent_search').split('|')
            data_array.reverse()
        ...
        if 'search_word' in self.params['param']:
            ...
            if skbd.isConfirmed():
                ...
                addon.setSetting('torrent_search', data_array)
            ...
            for data in data_array:
                try:
                    ...
                    if '<div id=\'torrent_' in data:
                        ...
                        self.create_line(title=label, params={'mode': 'bencode_part', 'torrent_id': torrent_id, 'anime_id': self.params['id'], 'info_data': self.params['param']}, **info)
                except:
...
#========================#========================#========================#
    def exec_bencode_part(self):
        ...
        # torrent_url = 'https://tr.anidub.com/engine/download.php?id={}'.format(torrent_id)
        torrent_url = '{}engine/download.php?id={}'.format(self.site_url, torrent_id)
