...
#================================================
        from anilibria.network import Anilibria_WEB
        self.network = Anilibria_WEB(
...
#================================================
        from anilibria.database import Anilibria_DB
...
#================================================
    def create_proxy_data(self):
        if Anilibria.addon.getSetting('anilibria_unblock') == '0':
            ...
            try: proxy_time = float(Anilibria.addon.getSetting('anilibria_proxy_time'))
            except: proxy_time = 0

            if time.time() - proxy_time > 28800:
                Anilibria.addon.setSetting('anilibria_proxy_time', str(time.time()))
                ...
                Anilibria.addon.setSetting('anilibria_proxy', proxy)
                proxy_data = {'https': proxy}
            else:
                if Anilibria.addon.getSetting('anilibria_proxy'):
                    proxy_data = {'https': Anilibria.addon.getSetting('anilibria_proxy')}
                else:
                    proxy_pac = urlopen("http://antizapret.prostovpn.org/proxy.pac").read()
...
#================================================
    def create_site_url(self):
        if not Anilibria.addon.getSetting('anilibria_mirror'):
            try:
                xbmc.log(str('CREATE_SITE_URL_NODE'), xbmc.LOGFATAL)
                self.exec_mirror_part()
                site_url = '{}public/api/index.php'.format(Anilibria.addon.getSetting('anilibria_mirror'))
            except:
                site_url = "https://www.anilibria.tv/public/api/index.php"
        else:
            site_url = '{}public/api/index.php'.format(Anilibria.addon.getSetting('anilibria_mirror'))           
        return site_url
#================================================  
    def create_post(self):
        ...
        if self.params['param'] == 'catalog':
            post = urlencode({"query": "catalog", "page": self.params['page'], "filter":"id,series,announce", "xpage": "catalog",
                    ...
                    "season": Anilibria.addon.getSetting('anilibria_season')},"sort": info.sort[Anilibria.addon.getSetting('anilibria_sort')],
                    "finish":info.status[Anilibria.addon.getSetting('anilibria_status')]})
    ...
    def exec_mirror_part(self):
        from anilibria.network import Anilibria_WEB
        self.net = Anilibria_WEB(auth_usage=False,auth_status=False,proxy_data=None)
        del Anilibria_WEB
    ...
    def exec_catalog_part(self):
        ...
        if self.params['param'] == 'genre':
            ...
            Anilibria.addon.setSetting(id='anilibria_genre', value=info.genre[result])
        
        if self.params['param'] == 'year':
            result = self.dialog.select('Год:', info.year)
            Anilibria.addon.setSetting(id='anilibria_year', value=info.year[result])
        
        if self.params['param'] == 'season':
            result = self.dialog.select('Сезон:', info.season)
            Anilibria.addon.setSetting(id='anilibria_season', value=info.season[result])

        if self.params['param'] == 'sort':
            result = self.dialog.select('Сортировать по:', tuple(info.sort.keys()))
            Anilibria.addon.setSetting(id='anilibria_sort', value=tuple(info.sort.keys())[result])
        
        if self.params['param'] == 'status':
            result = self.dialog.select('Статус релиза:', tuple(info.status.keys()))
            Anilibria.addon.setSetting(id='anilibria_status', value=tuple(info.status.keys())[result])
