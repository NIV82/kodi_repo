...
class DataBase:
    ...
    def add_serial(self, info={}):
        ...
        # if not update:
        #     self.cu.execute('INSERT INTO rhs_db (serial_id, title_ru, title_en, aired_on, genres, studios, country, description, image_id) VALUES (?,?,?,?,?,?,?,?,?)',
        #                     (serial_id, title_ru, title_en, aired_on, genres, studios, country, description, image_id))
        # else:
        #     self.cu.execute('UPDATE rhs_db SET title_ru=?, title_en=?, aired_on=?, genres=?, studios=?, country=?, description=?, image_id=? WHERE serial_id=?',
