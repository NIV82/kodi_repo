...
class Main:
    addon = xbmcaddon.Addon(id='plugin.shiza')
    ...
    def __init__(self):
        ...
        if not os.path.isfile(os.path.join(self.addon_data_dir, 'shiza.db')):
            try:
                data = urllib.urlopen('https://github.com/NIV82/kodi_repo_py2/raw/master/release/plugin.shiza/shiza.db')
                chunk_size = 8192
                bytes_read = 0
    ...
    def create_line(self, title=None, params=None, anime_id=None, size=None, folder=True): 
        ...
        if self.params['mode'] == 'search_part' and self.params['param'] == '':
            li.addContextMenuItems([('[B]Очистить историю[/B]', 'Container.Update("plugin://plugin.shiza/?mode=clean_part")')])
