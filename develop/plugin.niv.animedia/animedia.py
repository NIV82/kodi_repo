...
###========================================================  quote_plus(cover[0])
    ...
    def create_line(self, title=None, params=None, anime_id=None, size=None, folder=True, ex_info=None): 
        ...
        if self.params['mode'] == 'information_part':
            li.addContextMenuItems([('[B]Обновить Базу Данных[/B]', 'Container.Update("plugin://plugin.niv.animedia/?mode=update_part")')])
    ...
    def exec_update_part(self):
        ...
        try:                
            ...
            with open(db_file, 'wb') as write_file:
                ...
                self.progress.close()
            self.dialog.ok('База Данных','БД успешно загружена')
            Main.addon.setSetting('database', 'true')
        except:
            self.dialog.ok('База Данных','Ошибка загрузки - [COLOR=yellow]ERROR: 100[/COLOR])')
