...
#================================================
        if not Main.addon.getSetting("login") or not Main.addon.getSetting("password"):
            ...
            self.dialog.ok('AniDUB', 'Ошибка авторизации\nУкажите логин и пароль')
            return

        if not self.network.auth_status:
            if not self.network.authorization():
                self.params['mode'] = 'addon_setting'
                #xbmc.executebuiltin('XBMC.Notification(Ошибка авторизации, Проверьте логин и пароль)')
...
#================================================
    ...
    def create_title_info(self, title):
        ...
        title = utility.tag(title).replace('...','')
        ...
        try:
            info['series'] = v[0]
            info['title_ru'] = utility.rep(v[1]).capitalize()
            info['title_en'] = utility.rep(v[2]).capitalize()
        except:
            pass
        return info
    ...
    def create_info(self, anime_id):
        ...
        data_array = utility.clean(data_array).split('<br>')
        ...
        for data in data_array:
            if 'Год: </b>' in data:
                for year in range(1950, 2030, 1):
                    if str(year) in data:
                        info['year'] = year
            if 'Жанр: </b>' in data:
                genre = utility.tag(data.replace('Жанр: </b>', ''))
                info['genre'] = utility.rep(genre).lower()
            if 'Страна: </b>' in data:
                info['country'] = utility.tag(data.replace('Страна: </b>', ''))
            ...
            if '<b itemprop="director"' in data:
                director = utility.tag(data.replace('Режиссер: </b>', ''))
                info['director'] = utility.rep(director)
            if '<b itemprop="author"' in data:
                writer = utility.tag(data.replace('Автор оригинала / Сценарист: </b>', ''))
                info['writer'] = utility.rep(writer)
            if 'Озвучивание: </b>' in data:
                dubbing = utility.tag(data.replace('Озвучивание: </b>', ''))
                info['dubbing'] = utility.rep(dubbing)
            if 'Перевод: </b>' in data:
                translation = utility.tag(data.replace('Перевод: </b>', ''))
                info['translation'] = utility.rep(translation)
            if 'Тайминг и работа со звуком: </b>' in data:
                sound = utility.tag(data.replace('Тайминг и работа со звуком: </b>', ''))
                info['sound'] = utility.rep(sound)
            if 'Студия:</b>' in data:
                studio = data[data.find('xfsearch/')+9:data.find('/">')]
                info['studio'] = utility.rep(studio)
            if 'Описание:</b>' in data:
                plot = utility.tag(data.replace('Описание:</b>', ''))
                info['plot'] = utility.rep(plot)
        try:
    ...
    def exec_select_part(self):
        ...
        for data in data_array:
            ...
            if '<div id=\'torrent_' in data:
                ...
                size = data[data.find('Размер: <span class="red">')+26:data.find('</span> <span class="sep"></span> Скачали')]
    ...
    def exec_torrent_part(self):
        ...
        if b'files' in info:
            ...
            self.create_line(title=info[b'name'], params={'mode': 'play_part', 'index': 0, 'id': self.params['torrent_id']}, anime_id=self.params['id'], folder=False, size=info[b'length'])
