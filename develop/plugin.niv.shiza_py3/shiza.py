...
#================================================
        ...
        if time.time() - session_time > 28800:
            ...
            xbmcvfs.delete(os.path.join(self.cookies_dir, 'shiza.sid' ))
...
#================================================
    ...
    def create_info(self, anime_id):
        ...
        info['title_ru'] = utility.rep(title[0]).strip()
        info['title_en'] = utility.rep(title[1].split(',',1)[0]).strip()

        html = utility.clean(html)

        # title = html[html.find('"alt="')+6:html.find('"title=')]
        
        # info['title_ru'] = utility.rep(html[html.rfind('<h3>')+4:html.rfind('</h3>')])        
        # info['title_en'] = utility.rep(title[:title.find(',')])        
        ...
        info['plot'] = utility.rep(info['plot'])
        info['plot'] = utility.tag(info['plot']).strip()
        ...
        for data in data_array:
            if 'Жанры:' in data:
                info['genre'] = utility.tag(data).replace('Жанры:', '').strip()
            if 'Выпуск' in data:
                for i in range(1970, 2026, 1):
                    if str(i) in data:
                        info['year'] = i
            if 'Cоздатели' in data:
                info['authors'] = utility.tag(data).replace('Cоздатели:', '').strip()
            if 'Студия' in data:
                info['studio'] = utility.tag(data).replace('Студия:', '').strip()
            if 'Метки' in data:
                if info['genre'] == '':
                    info['genre'] = utility.tag(data).replace('Метки:', '').strip()
    ...
    def create_exinfo(self, torr_id, ex_info):
        ...
        for ex in ex_info:
            if torr_id in ex:
                ...
                seed_peer = utility.tag(seed_peer).split(',')
